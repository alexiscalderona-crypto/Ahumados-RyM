...
<main class="pt-24 px-6 pb-12 max-w-3xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="p-6 border-b border-gray-200">
            <h2 class="text-2xl font-bold">Añadir Nuevo Producto</h2>
        </div>
        <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            ...
            <div>
                ...
                <input type="text" name="title" required class="w-full border-gray-300 rounded p-3 focus:border-primary focus:ring-0" value="{{ old('title') }}">
                @error('title') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
            </div>

            <div>
                <label class="block font-bold mb-2">Descripción</label>
                <textarea name="description" rows="4" required class="w-full border-gray-300 rounded p-3 focus:border-primary focus:ring-0">{{ old('description') }}</textarea>
            ...
            <div class="grid grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" step="0.01" name="price" required class="w-full border-gray-300 rounded p-3 focus:border-primary focus:ring-0" value="{{ old('price') }}">
                    @error('price') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label class="block font-bold mb-2">Stock Disponible</label>
                    <input type="number" name="stock" required class="w-full border-gray-300 rounded p-3 focus:border-primary focus:ring-0" value="{{ old('stock') }}">
            ...
            <div>
                ...
                <input type="file" name="image" accept="image/*" required class="w-full border border-gray-300 rounded p-3 bg-gray-50">
                <p class="text-sm text-gray-500 mt-1">Sube una imagen de alta calidad desde tu computadora (Max: 2MB).</p>
            ...
            <div class="pt-4 flex justify-end gap-4">
                ...
                <button type="submit" class="px-6 py-3 bg-primary text-white font-bold rounded hover:bg-red-900 transition">Guardar Producto</button>
            </div>
        </form>
    </div>
</main>
