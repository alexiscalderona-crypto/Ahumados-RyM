...
<head>
    ...
    <title>Ahumados R y M | Crear Cuenta</title>
        ...
        <section class="w-full md:w-1/2 min-h-screen flex items-center justify-center px-6 py-20 bg-surface">
            <div class="w-full max-w-md">
                <div class="flex justify-center space-x-8 mb-12 border-b border-outline/10">
                    <a href="{{ route('login') }}" class="pb-4 font-label-lg border-b-2 transition-all duration-300 border-transparent text-on-surface-variant hover:text-gray-900">
                        LOGIN
                    </a>
                    <a href="{{ route('register') }}" class="pb-4 font-label-lg border-b-2 transition-all duration-300 border-primary text-primary">
                        REGISTER
                    </a>
                </div>
                ...
                <div class="space-y-8">
                    <div class="text-center md:text-left">
                        <h3 class="font-headline-md text-3xl mb-1">Join the Heritage.</h3>
                        <p class="text-on-surface-variant">Create an account to access exclusive smokehouse releases.</p>
                    </div>

                    <form method="POST" action="{{ route('register') }}" class="space-y-6">
                        @csrf
                        <div class="space-y-1 group">
                            <label class="font-label-sm text-xs text-on-surface-variant group-focus-within:text-primary transition-colors" for="name">FULL NAME</label>
                            <input class="w-full bg-transparent border-0 border-b border-outline/20 focus:ring-0 focus:border-primary transition-all py-2 px-0 font-body-md placeholder-outline-variant" id="name" type="text" name="name" value="{{ old('name') }}" required autofocus />
                        ...
                        <div class="space-y-1 group">
                            ...
                            <input class="w-full bg-transparent border-0 border-b border-outline/20 focus:ring-0 focus:border-primary transition-all py-2 px-0 font-body-md placeholder-outline-variant" id="email" type="email" name="email" value="{{ old('email') }}" required />
                            <x-input-error :messages="$errors->get('email')" class="mt-2 text-red-600 text-sm" />
                        </div>

                        <div class="space-y-1 group">
                            <label class="font-label-sm text-xs text-on-surface-variant group-focus-within:text-primary transition-colors" for="password">CREATE PASSWORD</label>
                            <input class="w-full bg-transparent border-0 border-b border-outline/20 focus:ring-0 focus:border-primary transition-all py-2 px-0 font-body-md placeholder-outline-variant" id="password" type="password" name="password" required autocomplete="new-password" />
                            <x-input-error :messages="$errors->get('password')" class="mt-2 text-red-600 text-sm" />
                        </div>

                        <div class="space-y-1 group">
                            <label class="font-label-sm text-xs text-on-surface-variant group-focus-within:text-primary transition-colors" for="password_confirmation">CONFIRM PASSWORD</label>
                            <input class="w-full bg-transparent border-0 border-b border-outline/20 focus:ring-0 focus:border-primary transition-all py-2 px-0 font-body-md placeholder-outline-variant" id="password_confirmation" type="password" name="password_confirmation" required autocomplete="new-password" />
